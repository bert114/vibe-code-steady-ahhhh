...
- E2E journeys: `npm run test:e2e` (Playwright, Chromium). Spins its own stack
  (API `:5001` + client `:5174`, `CLIENT_ORIGIN` matched to the E2E client) with
  a dedicated E2E user created/removed by `e2e/global-setup|teardown.cjs`, so
  dev/tester data is never touched. Needs `DATABASE_URL` (reads
  `server/.env`). Intercept API calls by origin (`**/localhost:5001/api/**`) —
  a bare `**/api/**` also aborts Vite's `/src/lib/api/*.js` modules.
...
| Auth (Clerk) | Supertest with injected `getAuthFn` (valid/invalid/throwing, sub isolation, bypass precedence); production boot guards exit 1 without keys |
