...
**Primary (2026-09-19):** Groq free tier through its OpenAI-compatible chat
completions API with structured JSON output (`AI_PROVIDER=groq`,
`GROQ_MODEL` env-pinned). Supersedes the brief OpenAI pick the same day: the
owner requires no billing and no credit card anywhere, and OpenAI's API is
prepaid-only. Groq's free key needs no card, and API customer data is not used
for training under Groq's processor terms (prompts not retained). Supersedes
the 2026-09-18 Cloudflare pick: the owner has no Cloudflare account.

**Why:**

- $0 with no card: free key, ~30 req/min and hundreds–thousands of req/day per
  model — far above 10–20 testers at the 5/hour per-user rate limit.
- Groq does not train on API data and does not retain prompts.
- OpenAI-compatible endpoint, so the adapter mirrors the existing provider
  shape and the same Zod schema validates the output.
- It is called from the Express server over REST, so the client never needs
  the provider credential.

Dormant alternatives (`cloudflare.provider.js`, `openai.provider.js`): the
owner holds an OpenAI key (unused: prepaid-only, breaks the $0 rule); no
Cloudflare account exists. OpenAI notes, kept for a possible return: no
training on API data by default (opt-in only; 30-day abuse logs), native
Structured Outputs, `gpt-4o-mini` at $0.15/$0.60 per 1M tokens.

Cloudflare Workers AI remains as a dormant alternative (`cloudflare.provider.js`,
`AI_PROVIDER=cloudflare`) — unwired, no account exists:
...
Current docs also note that some models have moved to paid-only access, so model availability must be verified at implementation time. [Workers AI Pricing](https://developers.cloudflare.com/workers-ai/platform/pricing/) and [Workers AI Data Usage](https://developers.cloudflare.com/workers-ai/platform/data-usage/) — accessed September 18, 2026. OpenAI API data-use and `gpt-4o-mini` pricing/structured-output support verified September 19, 2026.
...
6. What exact authentication method should be enabled before the 10–20-user beta?
   → Decided 2026-09-19 (Phase 5): Clerk free Hobby (verified $0 to 50k MRU/app).
   `@clerk/express` clerkMiddleware + getAuth on the API (Bearer header for the
   cross-origin static client), `@clerk/clerk-react` provider + gates on the
   client, `auth_identities` mapping (003) from Clerk sub to internal UUID so no
   existing table changes. No Cloudflare account exists, so the client deploys
   as a Render Static Site instead of Cloudflare Pages (render.yaml).
7. What privacy notice and deletion flow should be shown to testers?
   → Decided 2026-09-19 (Phase 5): `/settings` page holds the ownership
   explanation, AI data-use explanation (honest fallback-only status while no
   provider is configured), and the not-a-diagnosis statement; `DELETE
   /api/users/me` wipes the account via existing ON DELETE CASCADEs after a
   two-step confirm. Tester feedback is an external form linked from the
   dashboard (`VITE_FEEDBACK_URL`), not a backend surface.
