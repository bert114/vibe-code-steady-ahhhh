...
- For boundary-type insights, use two short parts: an "Observed:" line grounded in the evidence, then a "Reflection:" line phrased as a gentle noticing prompt. Example — Observed: "Time around overtime has felt draining 3 times this week." Reflection: "You may want to notice whether you are saying yes when you would rather say no."
...
function maxInputChars() {
  // process.env values are always strings; an env var set to an EMPTY
  // string (e.g. "AI_MAX_INPUT_CHARS=" with nothing after the `=`) is not
  // null/undefined, so `??` alone won't fall back — Number('') is 0, which
  // truncated every check-in payload to nothing before it reached the AI.
  const raw = process.env.AI_MAX_INPUT_CHARS
