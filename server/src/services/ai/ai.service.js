...
// deps.provider defaults to Cloudflare; tests inject fakes with the same
// async ({ system, user }) => string signature.
export async function analyzeRecentCheckins(userId, deps = {}) {
  const runProvider = deps.provider ?? cloudflareRun
  ...
  if (reminder?.shouldShow && reminder.message && stored[0]) {
    await repository.insertReminder(userId, stored[0].id, 'insight', reminder.message)
  }
