    ...
    return (
      ...
      >
        <div className="battery-head">
          <p id="battery-label" className="battery-title">Current Energy</p>
        </div>
        <div className="battery-body">
          <EnergyBattery checkins={checkins} />
          <p className="battery-note">
            No reading yet. <Link className="dashboard-panel__link" to="/check-in">Record a check-in</Link>
          </p>
        </div>
      </section>
    )
  }
  const updated = new Date(latest.occurredAt).toLocaleDateString(undefined, { month: 'short', day: 'numeric' })
  return (
    <section
      className="dashboard-panel dashboard-panel--battery-compact reveal"
      style={{ '--index': 1 }}
...
function LatestInsight({ insight }) {
  if (!insight) {
    return <p>No insights yet. Record a few check-ins, then run an analysis.</p>
  }
  return (
    <article className="dashboard-panel dashboard-panel--feature">
      {insight.confidence && <p className={confidenceTag(insight.confidence)}>{insight.confidence}</p>}
      <h3>{insight.title}</h3>
      <p>{insight.summary}</p>
      <p>
...
export default function DashboardPage() {
  const { summary, status, error, markingId, load, markRead } = useDashboardStore()
    ...
    <main className="dashboard-page" ref={scopeRef}>
      <div className="dashboard-page__heading reveal" style={{ '--index': 0 }}>
        <div>
          ...
          <p>Come back to yourself without needing to solve everything at once.</p>
        </div>
        <p className="dashboard-page__date">
          Today · {new Date().toLocaleDateString(undefined, { month: 'short', day: 'numeric' })}
        </p>
      </div>
      {error && <p role="alert">Couldn&apos;t load your dashboard right now: {error}</p>}
      {loading ? (
        ...
        summary && (
          <>
            ...
            <div className="dashboard-page__side">
              ...
              <div className="dashboard-panel reveal" style={{ '--index': 4 }}>
